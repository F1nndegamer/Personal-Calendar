/**
 * Google Calendar HTTP routes (server-side).
 *
...
export function isGooglePath(url: string): boolean {
...
function frontendBase(req: IncomingMessage): string {
...
async function validAccessToken(
  auth: GoogleAuthData, config: GoogleOAuthConfig,
  writeAuth: (d: GoogleAuthData) => void, fetchImpl: FetchImpl,
): Promise<string | null> {
...
async function buildStatus(
  config: GoogleOAuthConfig, auth: GoogleAuthData,
  writeAuth: (d: GoogleAuthData) => void, fetchImpl: FetchImpl,
): Promise<GoogleStatusResponse> {
  if (!auth.tokens) return { connected: false, calendars: [], selectedCalendarIds: [] };
...
function selectedCalendars(auth: GoogleAuthData): string[] {
...
export async function handleGoogleRequest(
  req: IncomingMessage, res: ServerResponse, url: string, deps: GoogleRouteDeps = {},
): Promise<boolean> {
  const path = url.split('?')[0];
  if (!isGooglePath(url)) return false;
  ...
  if (!config) {
    if (path === '/api/google/status' && req.method === 'GET') {
      ...
      } satisfies GoogleStatusResponse);
      return true;
    }
  ...
  if (path === '/api/google/callback' && req.method === 'GET') {
    ...
    try {
      ...
      try { email = await fetchAccountEmail(tokens.access_token, fetchImpl); } catch { email = undefined; }
      const prev = readAuth();
      writeAuth({
        ...prev,
        tokens: {
  ...
  if (path === '/api/google/logout' && req.method === 'POST') {
    ...
    void (async () => {
      try {
        ...
        if (refresh) await revokeToken(refresh, fetchImpl);
      } catch { /* ignore */ }
    })();
  ...
  if (path === '/api/google/selection' && req.method === 'POST') {
    ...
    const ids = (parsed as { calendarIds?: unknown }).calendarIds;
  ...
  if (path === '/api/google/events' && req.method === 'GET') {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        const code = (err as { googleApiError?: { code?: number } })?.googleApiError?.code;
  ...
  if (path === '/api/google/events' && req.method === 'POST') {
    ...
    const body = parsed as { title?: unknown; start?: unknown; end?: unknown; description?: unknown; calendarId?: unknown };
