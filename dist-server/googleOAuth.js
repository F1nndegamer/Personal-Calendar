...
export function newOAuthState(bytes = 16) {
    const buf = new Uint8Array(bytes);
    crypto.getRandomValues(buf);
    return Array.from(buf, (b) => b.toString(16).padStart(2, '0')).join('');
}
/** Authorization URL the browser is redirected to. */
export function buildAuthUrl(config, scopes, state) {
...
async function postForm(url, params, fetchImpl, what) {
    ...
    if (!res.ok) {
        throw Object.assign(new Error(`${what} failed with HTTP ${res.status}`), {
            googleApiError: { code: res.status, message: `${what} failed with HTTP ${res.status}` },
        });
    }
...
function apiError(what, status) {
    return Object.assign(new Error(`${what} failed with HTTP ${status}`), {
        googleApiError: { code: status, message: `${what} failed with HTTP ${status}` },
    });
}
...
export async function listCalendars(accessToken, fetchImpl = fetch) {
    ...
    if (!res.ok)
        throw apiError('Calendar list', res.status);
    const json = (await res.json());
...
export async function fetchAccountEmail(accessToken, fetchImpl = fetch) {
...
export async function listEventsPage(accessToken, calendarId, timeMin, timeMax, pageToken, fetchImpl = fetch) {
    ...
    const res = await fetchImpl(`${GOOGLE_API_BASE}/calendars/${encodeURIComponent(calendarId)}/events?${params.toString()}`, { headers: { Authorization: `Bearer ${accessToken}` } });
    if (!res.ok)
        throw apiError('Events list', res.status);
    const json = (await res.json());
    const items = Array.isArray(json.items) ? json.items : [];
...
export async function listAllEvents(accessToken, calendarId, timeMin, timeMax, fetchImpl = fetch) {
    const all = [];
    let page;
    for (let i = 0; i < 20; i++) {
        const { items, nextPageToken } = await listEventsPage(accessToken, calendarId, timeMin, timeMax, page, fetchImpl);
        all.push(...items);
        if (!nextPageToken)
            break;
        page = nextPageToken;
...
export async function createGoogleEvent(accessToken, calendarId, body, fetchImpl = fetch) {
    const res = await fetchImpl(`${GOOGLE_API_BASE}/calendars/${encodeURIComponent(calendarId)}/events`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
    });
    if (!res.ok)
        throw apiError('Event create', res.status);
    const json = (await res.json());
    if (typeof json.id !== 'string')
        throw new Error('Event create returned no id');
    return json.id;
}
/** Update an event. */
export async function updateGoogleEvent(accessToken, calendarId, eventId, body, fetchImpl = fetch) {
    const res = await fetchImpl(`${GOOGLE_API_BASE}/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(eventId)}`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
    });
    if (!res.ok)
        throw apiError('Event update', res.status);
}
/** Delete an event (404/410 = already gone, not an error). */
export async function deleteGoogleEvent(accessToken, calendarId, eventId, fetchImpl = fetch) {
    const res = await fetchImpl(`${GOOGLE_API_BASE}/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(eventId)}`, { method: 'DELETE', headers: { Authorization: `Bearer ${accessToken}` } });
