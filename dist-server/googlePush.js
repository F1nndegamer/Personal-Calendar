 ...
 */
export function parsePushBody(parsed) {
...
function apiErrorCode(err) {
    return err?.googleApiError?.code;
}
 ...
 */
async function runOp(op, id, stats, sleep) {
 ...
 */
export async function syncPushedEvents(input) {
            ...
            if (existing) {
                ...
                if (needsMove) {
                    ...
                    const created = await runOp(async () => {
                        const eventId = await createGoogleEvent(input.accessToken, input.calendarId, toGoogleEventBody(ev), input.fetchImpl);
                        next[ev.id] = { calendarId: input.calendarId, eventId, key };
                    }, ev.id, stats, sleep);
                    if (created) {
                        // A move counts as one create (new copy) + one delete (old copy).
                        stats.created++;
                        const gone = await runOp(() => deleteGoogleEvent(input.accessToken, existing.calendarId, existing.eventId, input.fetchImpl), ev.id, stats, sleep);
                        if (gone)
                            stats.deleted++;
                    }
                    return;
                }
                const updated = await runOp(() => updateGoogleEvent(input.accessToken, existing.calendarId, existing.eventId, toGoogleEventBody(ev), input.fetchImpl), ev.id, stats, sleep);
                if (updated) {
...
function allDayInstant(date, isEnd) {
...
function mirroredInstant(value, other, isEnd) {
...
export async function sweepOwnCopies(input) {
    ...
    for (const ev of input.events) {
        ...
        maxEnd = Math.max(maxEnd, isAllDayDate(ev.end) && isAllDayDate(ev.start)
